...
function formatReference(ref, subject) {
    ...
    // This ensures publishers are removed even if the logic below misses them
    ...
    publishersToRemove.forEach(publisher => {
        // Remove if it appears at the end after a comma or period
        const patterns = [
    ...
    // Check if author and publisher are the same (case-insensitive comparison)
    ...
    // Build formatted reference (APA 7 compliant)
    ...
    if (titlePart) {
        ...
        // Now add the period in italics
        formatted += ' <em>' + titlePart + '.</em>';
    }

    // ONLY add publisher if it's different from author

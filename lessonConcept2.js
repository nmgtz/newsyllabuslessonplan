(function() {
  const allowedHost = atob("YWNhamVuaXBvaW50LmNvbQ==");

// Normalize the hostname by removing "www." if present
const normalizedHost = window.location.hostname.replace(/^www\./, '');

// Check if the current hostname matches the allowed host or if the URL doesn't include the specific path
if (normalizedHost !== allowedHost || !window.location.href.includes("/p/create-lesson-plan.html")) {
  ...
  if (window.top !== window.self) {
      console.error("Your Domain has been Blocked from running in an iframe with this codes!");
      document.body.innerHTML = "";
  ...
  function detectDevTools() {
      ...
      if (devtools.opened) {
          console.warn("DevTools detected! Blocking execution...");
          document.body.innerHTML = "";
          throw new Error("Script execution blocked due to DevTools");
      }
  }

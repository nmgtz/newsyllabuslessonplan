...
function createFolderViewContainer() {
    ...
    const folderHTML = `
        <div id="lessonPlanFolder" class="lesson-folder-container compact-folder" style="display: none;">
            <div class="folder-header compact-header">
                <div class="folder-title">
                    <span class="folder-icon">📁</span>
                    <h3>My Lesson Plans</h3>
                    <span class="folder-count" id="folderCount">(0)</span>
                </div>
                <div class="folder-actions compact-actions">
                    <button class="folder-btn compact-btn" onclick="sortLessonPlans('date')" title="Sort by Date">📅</button>
                    <button class="folder-btn compact-btn" onclick="sortLessonPlans('name')" title="Sort by Name">🔤</button>
                    <button class="folder-btn compact-btn" onclick="createNewLessonPlan()" title="Create New">➕</button>
                    <button class="folder-btn compact-btn close-folder-btn" onclick="closeFolderView()" title="Close Folder">✖️</button>
                </div>
            </div>
            <div class="folder-content compact-content" id="folderContent">
                <div class="empty-folder compact-empty" id="emptyFolder">
...
function displayFolderContents() {
    ...
    let planListHTML = '<div class="lesson-plans-list compact-list">';
    
    savedPlans.forEach((plan, index) => {
        const planDate = new Date(plan.date).toLocaleDateString();
        const saveTime = new Date(plan.timestamp).toLocaleString();
        const displayName = plan.name.replace(/\s*\[\d+\]$/, ''); // Remove timestamp from display
        
        planListHTML += `
            <div class="lesson-plan-item compact-item" data-index="${index}">
                <div class="item-main" onclick="loadLessonPlan('${plan.name}')">
                    <div class="item-title">📄 ${displayName}</div>
                    <div class="item-meta">
                        <span class="meta-date">📅 ${planDate}</span>
                        <span class="meta-subject">${plan.data.subject || 'N/A'}</span>
                        <span class="meta-class">${plan.data.classes || 'N/A'}</span>
                    </div>
                </div>
                <div class="item-actions compact-actions">
                    <button class="action-btn edit-btn" onclick="editLessonPlan('${plan.name}')" title="Edit">✏️</button>
                    <button class="action-btn download-btn" onclick="downloadSavedPlan('${plan.name}')" title="Download">📥</button>
                    <button class="action-btn delete-btn" onclick="deleteLessonPlan('${plan.name}')" title="Delete">🗑️</button>
                </div>
            </div>
        `;
    });
...
function saveLessonPlanToFolder() {
    ...
    // Always save as new - no duplicate checking for unique lessons
    ...
    hideFolderView();
    showMainInterface();
    showLessonNotification("📝 Returned to lesson creation", "info");
}
...
function showMainInterface() {
    ...
    const selectors = [
        '.container > *:not(#lessonPlanFolder)',
        '.main-content',
        '.form-container',
        '.input-group',
        '.form-section'
    ...
    selectors.forEach(selector => {
        ...
        elements.forEach(element => {
            if (element && element.id !== 'lessonPlanFolder') {
                element.style.display = '';
            }
        });
    });
    
    // Show lesson form specifically
    showLessonForm();
}
...
function generateLessonName(formData) {
    ...
    // Create unique identifier with timestamp to avoid duplicates
    return `${subject} - ${specificLesson} - ${classes} (${date}) [${timestamp}]`;
}
...
function hideAllMainContent() {
    ...
    const selectors = [
        '.container > *:not(#lessonPlanFolder):not(.folder-toggle-btn)',
        '.main-content',
        '.lesson-content',
        '.lesson-template',
        '.form-container',
    ...
    selectors.forEach(selector => {
        const elements = document.querySelectorAll(selector);
        elements.forEach(element => {
            if (element && element.id !== 'lessonPlanFolder' && !element.classList.contains('folder-toggle-btn')) {
                element.style.display = 'none';
            }
        });
    });
...
function showFolderView() {
    ...
    // Show only the folder
    const folderElement = document.getElementById('lessonPlanFolder');
    if (folderElement) {
        folderElement.style.display = 'block';
        folderElement.style.visibility = 'visible';
    }
    
    // Ensure folder content is expanded
    const folderContent = document.querySelector('.folder-content');
    if (folderContent) {
        folderContent.style.display = 'block';
    }
...
function addFolderSystemStyles() {
    ...
    style.textContent = `
        ...
        .meta-date, .meta-subject, .meta-class {
            background: white;
            padding: 2px 6px;
            border-radius: 4px;

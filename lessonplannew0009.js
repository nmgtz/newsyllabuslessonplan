...
function formatReference(ref) {
    ...
    // Look for the pattern: AUTHOR. (YEAR). Title
    var periodIndex = ref.indexOf('.');
    if (periodIndex !== -1) {
        // Extract author (everything before first period)
        var authorPart = ref.substring(0, periodIndex).trim().toUpperCase();
        
        // Extract the rest (year and title)
        var restPart = ref.substring(periodIndex + 1).trim();
        
        // Check if restPart starts with a year pattern like (2023)
        var yearMatch = restPart.match(/^\((\d{4})\)\.\s*/);
        if (yearMatch) {
            var year = yearMatch[0]; // e.g., "(2023). "
            var titlePart = restPart.substring(yearMatch[0].length).trim();
            
            // Convert title to sentence case (first letter caps, rest lowercase, except after periods)
            titlePart = sentenceCase(titlePart);
            
            restPart = year + titlePart;
        } else {
            // If no year pattern, just apply sentence case to the whole rest
            restPart = sentenceCase(restPart);
        }
        
        return '<strong>' + authorPart + '</strong>, <em>' + restPart + '</em>';
    } else {

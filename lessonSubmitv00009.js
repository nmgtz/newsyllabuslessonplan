...
lessonplanSubmit.addEventListener('click', function (mySb) {
    ...
    panel.innerHTML = `
      ...
      <div class="panel-body">
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px;">
          <button id="downloadPdfBtn" class="sow-tool-btn primary" style="flex:1;justify-content:center;">
            <i class="bi bi-download"></i> Download PDF
          </button>
          <button id="backBtn" class="sow-tool-btn neutral" style="flex:1;justify-content:center;">
            <i class="bi bi-arrow-left-circle"></i> Back to Saved Data
          </button>
          <button id="newLessonBtn" class="sow-tool-btn accent" style="flex:1;justify-content:center;">
            <i class="bi bi-plus-circle"></i> New Lesson Plan
          </button>
          <button id="summaryBtn" class="sow-tool-btn success" style="flex:1;justify-content:center;">
    ...
    function _markAsDownloaded() {
        ...
        downloadBtn.style.background = 'var(--gold)';
    }
...
function buildAndDownloadPdf(filename, onSuccess) {
    ...
    const d = {
        ...
        lShule:      t('ShuleJina')         || 'SCHOOL NAME',
        school:      t('schoolNm'),
        ...
        lMudaDakika: t('muda-dakika')       || 'TIME (MIN)',
    ...
    doc.autoTable({
        ...
        pageBreak: 'avoid',
        rowPageBreak: 'avoid',
        styles:{ ...base, cellPadding:{ top:1.2, bottom:2, left:1, right:1 }, lineWidth:0 },
        columnStyles:{
        ...
        didDrawCell(data) {
            if (data.section==='body' && (data.column.index===1||data.column.index===3)) {
                ...
                doc.line(data.cell.x, data.cell.y+data.cell.height-0.4,
                         data.cell.x+data.cell.width, data.cell.y+data.cell.height-0.4);
            }
        },
    });
    ...
    doc.autoTable({
        ...
        pageBreak: 'avoid',
        rowPageBreak: 'avoid',
        styles:{ ...base, cellPadding:{ top:2, bottom:2, left:1, right:1 }, halign:'center', valign:'middle', fontStyle:'bold' },
        head:[
    ...
    const REMARKS_FS      = 8.5;
    const REMARKS_LINE_H  = 4.5;
    const REMARKS_VPAD    = 2.5;
    const REMARKS_LABEL_W = 22;
    const remarkTextW     = CW - REMARKS_LABEL_W - 3;

    doc.setFontSize(REMARKS_FS);
    const remarkLines = doc.splitTextToSize(d.remarkComm || '', remarkTextW);
    const remarkRowH  = Math.max(10, REMARKS_VPAD * 2 + remarkLines.length * REMARKS_LINE_H);
    const REMARKS_RESERVE = remarkRowH + 2;

    const availH = (PH - MB - REMARKS_RESERVE) - y;

    const HDR_ROW_H   = 8;
    const vPadFixed   = 1.5;
    const hPadFixed   = 2;
    const ROW_WEIGHTS = [0.80, 1.20, 1.20, 0.80];
    const weightSum   = ROW_WEIGHTS.reduce((a, b) => a + b, 0);
    const unitH       = (availH - HDR_ROW_H) / weightSum;
    const rowHeights  = ROW_WEIGHTS.map(w => w * unitH);

    const pCW = [28, 14, 52, 52, CW - 28 - 14 - 52 - 52];

    function estLines(text, colWidthMm, fontSizePt) {
        if (!text) return 1;
        const ptPerMm      = 2.835;
        const charWidthMm  = (fontSizePt * 0.52) / ptPerMm;
        const charsPerLine = Math.max(1, Math.floor(colWidthMm / charWidthMm));
        const words = text.split(/\s+/);
        let lines = 1, cur = 0;
        words.forEach(w => {
            if (cur + w.length + 1 > charsPerLine && cur > 0) { lines++; cur = 0; }
            cur += w.length + 1;
        });
        return lines;
    }

    function maxLinesInRow(rowCells, fontSizePt) {
        const cw = pCW.map(w => w - hPadFixed * 2);
        let max = 1;
        rowCells.forEach((cell, ci) => {
            const n = estLines(cell, cw[ci], fontSizePt);
            if (n > max) max = n;
        });
        return max;
    }

    function fontFitsRow(fontSizePt, rowCells, rh) {
        const ptPerMm  = 2.835;
        const lineH    = (fontSizePt / ptPerMm) * 1.15 + 0.5;
        const usableH  = rh - vPadFixed * 2;
        return (maxLinesInRow(rowCells, fontSizePt) * lineH) <= usableH;
    }
    ...
    const fontScale = [10.0, 9.5, 9.0, 8.5, 8.0, 7.5, 7.0, 6.5, 6.0, 5.5, 5.0, 4.5];

    function pickFontForRow(rowCells, rh) {
        for (const fs of fontScale) {
            if (fontFitsRow(fs, rowCells, rh)) return fs;
        }
        return fontScale[fontScale.length - 1];
    }

    const perRowFonts = stageRows.map((r, i) => pickFontForRow(r, rowHeights[i]));
    const globalFont  = Math.min(...perRowFonts);
    ...
    doc.autoTable({
        ...
        styles: {
            ...base,
            fontSize:    8.5,
            cellPadding: { top: vPadFixed, bottom: vPadFixed, left: hPadFixed, right: hPadFixed },
            valign:      'middle',
            overflow:    'linebreak',
            lineWidth:   0.5,
        },
        ...
        columnStyles: {
            ...
            1: { cellWidth: pCW[1], halign:    'center', valign: 'middle' },
            2: { cellWidth: pCW[2] },
            3: { cellWidth: pCW[3] },
            4: { cellWidth: pCW[4] },
        },
        ...
        didParseCell(data) {
            if (data.section === 'body') {
                ...
                data.cell.styles.fontSize      = globalFont;
            }
        },
    });

    const stageBottom = doc.lastAutoTable.finalY;
    const remarksBoxY = stageBottom + 1;

    // Remarks label
    doc.setFont('helvetica','bold');
    doc.setFontSize(REMARKS_FS);
    doc.setTextColor(...BLACK);
    doc.text(lbl(d.lMaoni), ML + 1.5, remarksBoxY + REMARKS_VPAD + REMARKS_LINE_H - 1);

    // Remarks content
    doc.setFont('helvetica','normal');
    doc.setFontSize(REMARKS_FS);
    remarkLines.forEach((line, i) => {
        doc.text(line, ML + REMARKS_LABEL_W + 2, remarksBoxY + REMARKS_VPAD + REMARKS_LINE_H - 1 + (i * REMARKS_LINE_H));
    });

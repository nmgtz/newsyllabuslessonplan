...
function buildAndDownloadPdf(filename, onSuccess) {
    ...
    const d = {
        ...
        lAngalia:    t('angalia')           || 'ASSESSMENT CRITERIA',
    ...
    doc.autoTable({
        ...
        columnStyles:{
            ...
            1:{ cellWidth:CW-compLblW, valign:'bottom', fontSize:9.0 },
        },
        body:[
 ...
 function formatListBreaks(text) {
        if (!text) return text;
        return text
            .replace(/\s*\(([a-z])\)\s*/g, (match, letter, offset) => {
                return (offset === 0 ? '' : '\n') + `(${letter}) `;
            })
            .trim();
    }
    ...
    const stageRows = [
        [ (d.lMwanzo     ||'').toUpperCase(), d.introCont, formatListBreaks(d.intro1),   formatListBreaks(d.intro2),   formatListBreaks(d.intro3)   ],
        [ (d.lMpya       ||'').toUpperCase(), d.newKnow,   formatListBreaks(d.new1),     formatListBreaks(d.new2),     formatListBreaks(d.new3)     ],
        [ (d.lKuimarisha ||'').toUpperCase(), d.reinCont,  formatListBreaks(d.rein1),    formatListBreaks(d.rein2),    formatListBreaks(d.rein3)    ],
        [ (d.lTafakari   ||'').toUpperCase(), d.refleCont, formatListBreaks(d.reflect1), formatListBreaks(d.reflect2), formatListBreaks(d.reflect3) ],
    ];

    const fontScale = [9.0, 8.5, 8.25, 7.5, 7.25, 7.0];

    
    function measureRowHeights(fontSizePt) {
        doc.setFontSize(fontSizePt);
        const lineH = (fontSizePt / ptPerMm) * 1.15 + 0.5;
        return stageRows.map(row => {
            let maxLines = 1;
            row.forEach((cell, ci) => {
                const colW = pCW[ci] - hPadFixed * 2;
                const lines = doc.splitTextToSize(cell || '', colW).length;
                if (lines > maxLines) maxLines = lines;
            });
            return Math.max(6, maxLines * lineH + vPadFixed * 2);
        });
    }

    
    let chosenFont = fontScale[fontScale.length - 1];
    let rowHeights;
    for (const fs of fontScale) {
        ...
        if (total <= availH) {
            chosenFont = fs;
            const leftover = availH - total;
            const bonus = leftover > 0 ? leftover / measured.length : 0;
...
function manualSaveLessonPlan() {
    ...
    const formData = getCurrentFormData();
    return saveCurrentPlan(formData, false);
}

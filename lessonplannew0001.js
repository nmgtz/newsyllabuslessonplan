...
function generateSwahiliPhrases(verb, specificLesson, isContinue) {
    ...
    const wrdImp2 = getValue('wrdImp2');
       ...
       var subDate = dateOption === "set" ? document.getElementById("date").value : "";
...
var totalMinutes = 0;
var introTime, devTime, designTime, realizationTime;
...
} else if (remarkOption === "notset") {
    remarkComm.innerText = "";
} else if (remarkOption === "auto") {
...
reflect3.innerText = phrases.reflect3;
    ...
    totalMinutes = parseInt(document.getElementById("customMinutes").value) || 40;
    const periodTypeVal = document.getElementById("periodType").value;

    if (periodTypeVal === "single") {
        introTime = Math.round(totalMinutes * 0.125);      // 12.5%
        devTime = Math.round(totalMinutes * 0.375);        // 37.5%
        designTime = Math.round(totalMinutes * 0.25);      // 25%
        realizationTime = totalMinutes - (introTime + devTime + designTime); 
    } else { // double
        introTime = Math.round(totalMinutes * 0.125);      // 12.5%
        devTime = Math.round(totalMinutes * 0.50);         // 50%
        designTime = Math.round(totalMinutes * 0.1875);    // 18.75%
        realizationTime = totalMinutes - (introTime + devTime + designTime); 
    }
    
} else if (period === "custom-distribution") {
    // Custom distribution
    introTime = parseInt(document.getElementById("introMinutes").value) || 5;
    devTime = parseInt(document.getElementById("developmentMinutes").value) || 15;
    designTime = parseInt(document.getElementById("designMinutes").value) || 10;
    realizationTime = parseInt(document.getElementById("realizationMinutes").value) || 10;
    totalMinutes = introTime + devTime + designTime + realizationTime;
    
} else if (period === "2") {
    // Standard double period
    totalMinutes = 80;
    introTime = 10;
    devTime = 40;
    designTime = 15;
...
// Set the time values in the PDF
introCont.innerText = introTime.toString();
newKnow.innerText = devTime.toString();
reinCont.innerText = designTime.toString();
refleCont.innerText = realizationTime.toString();

// Calculate lesson time (this part remains in both Swahili and English sections)
if (period !== "") {
    var startHour = document.getElementById("hourCodes").value;
    var startMinutes = document.getElementById("minCodes").value;
    const acaMinutes = totalMinutes;
...
reflect3.innerText = phrases.reflect3;
    ...
    if (periodTypeVal === "single") {
        introTime = Math.round(totalMinutes * 0.125);      
        devTime = Math.round(totalMinutes * 0.375);        
        designTime = Math.round(totalMinutes * 0.25);      
        realizationTime = totalMinutes - (introTime + devTime + designTime); 
    } else { // double
        introTime = Math.round(totalMinutes * 0.125);      
        devTime = Math.round(totalMinutes * 0.50);         
        designTime = Math.round(totalMinutes * 0.1875);    
        realizationTime = totalMinutes - (introTime + devTime + designTime); 
    }

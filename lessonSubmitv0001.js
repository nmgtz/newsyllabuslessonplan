    ...
    nextStep() {
        if (this.validateStep(this.currentStep)) {
            if (this.currentStep < this.totalSteps) {
                ...
                this.currentStep++;
                this.updateStep();
                const stepNames = ['','Basic Info','Lesson Details','Learning Objectives','Teaching & Materials','Review & Submit'];
                showStepNotification(prevStep + 1, this.totalSteps, stepNames[this.currentStep] || `Step ${this.currentStep}`);
            }
        }
    }
    prevStep() {
...
lessonplanSubmit.addEventListener('click', function (mySb) {
    ...
    function _doDownload(onSuccess) {
        ...
        downloadDocument(onSuccess);
    }
    function _markAsDownloaded() {
        _pdfDownloaded = true;
    ...
    downloadBtn.addEventListener('click', function() {
        if (!_pdfDownloaded) {
            _doDownload(_markAsDownloaded);
        } else {
            showConfirmationModal({
                title: "Download Again?",
                message: "You have already downloaded this PDF.\n\nDo you want to download it again as a backup copy?",
                confirmText: "Yes, Download Again",
    ...
    backBtn.addEventListener('click', function() {
        showConfirmationModal({
            title: "Go Back to Saved Data?",
            message: "Are you sure you want to go back?\n\nYour current lesson plan view will close. Any unsaved changes will be lost.",
...
function downloadDocument(onSuccess) {
    ...
    showDocumentNameModal(defaultName1, (documentName) => {
        ...
        setTimeout(() => {
            try {
                buildAndDownloadPdf(finalName, onSuccess);
            } catch(e) {
                hidePdfLoader();
...
function buildAndDownloadPdf(filename, onSuccess) {
    ...
    // ── Collect all values from rendered template ──
    function t(id) {
    ...
    const d = {
        ...
        // Details table
        lShule:       t('ShuleJina')        || 'School',
        school:       t('schoolNm'),
        lTeacher:     t('jinaMwalimu')      || "Teacher's Name",
        teacher:      t('lessonTch'),
        lClass:       t('darasa')           || 'Class',
        classVal:     t('lessonClss'),
        stream:       t('lessonStrm'),
        lSubject:     t('somo')             || 'Subject',
        subject:      t('lessonSbjt'),
        lTime:        t('muda')             || 'Time',
        time:         t('lessonTm'),
        lDate:        t('tarehe')           || 'Date',
        date:         t('lessonDt'),
        ...
        lGirls:       t('wasichana')        || 'Girls',
        lBoys:        t('wavulana')         || 'Boys',
        lTotal:       t('jumla')            || 'Total',
        lGirls1:      t('wasichana1')       || 'Girls',
        lBoys1:       t('wavulana1')        || 'Boys',
        lTotal1:      t('jumla1')           || 'Total',
        flReg:        t('flReg'),
        mlReg:        t('mlReg'),
        stRegt:       t('stRegt'),
        flPr:         t('flPr'),
        ...
        // Syllabus table
        lUjuzi:       t('ujuzi')            || 'Main Competence',
        lLengoKuu:    t('lengoKuu')         || 'Specific Competence',
        lMadaNdogo:   t('madaNdogo')        || 'Main Activity',
        lMahususi:    t('lengoMahususi')    || 'Specific Activities',
        lNukuu:       t('nukuu')            || 'T & L Resources',
        lRejea:       t('rejea')            || 'References',
        sylbsMnObj:   t('sylbsMnObj'),
        sylbsCmp:     t('sylbsCmp'),
        sylbsSbtp:    t('sylbsSbtp'),
        sylbsSpobj:   t('sylbsSpobj'),
        sylbsMts:     t('sylbsMts'),
        sylbsRfrs:    h('sylbsRfrs'),

        // Process table
        lKichwa:      t('kichwa-habari')    || 'Teaching and Learning Process',
        lHatua:       t('hatua')            || 'Stages',
        lMudaDakika:  t('muda-dakika')      || 'Time\n(Min)',
        lMwalimu:     t('mwalimu')          || 'Teaching Activities',
        lMwanafunzi:  t('mwanafunzi')       || 'Learning Activities',
        lAngalia:     t('angalia')          || 'Assessment',
        lMwanzo:      t('mwanzo')           || 'Introduction',
        lMpya:        t('mpya')             || 'Competence Development',
        lKuimarisha:  t('kuimarisha')       || 'Design',
        lTafakari:    t('tafakari')         || 'Realization',
        introCont:    t('introCont'),
        newKnow:      t('newKnow'),
        reinCont:     t('reinCont'),
        refleCont:    t('refleCont'),
        intro1:       t('intro1'),
        intro2:       t('intro2'),
        intro3:       t('intro3'),
        new1:         t('new1'),
        ...
        // Remarks
        lMaoni:       t('maoni')            || 'REMARKS',
        remarkComm:   t('remarkComm'),
    };
    ...
    // ─────────────────────────────────────────────
    // 2. DETAILS TABLE  —  class="details-tb"
    //    4-column layout: label | value | label | value
    //    value cells have dotted bottom border, no other borders
    // ─────────────────────────────────────────────
    const detailColW = [22, 73, 30, CW - 22 - 73 - 30]; // ~69mm last col
    ...
    doc.autoTable({
        ...
        columnStyles: {
            0: { fontStyle: 'bold', cellWidth: detailColW[0], textColor: BLACK, valign: 'bottom' },
            1: { cellWidth: detailColW[1],                    valign: 'bottom' },
            2: { fontStyle: 'bold', cellWidth: detailColW[2], textColor: BLACK, valign: 'bottom' },
            3: { cellWidth: detailColW[3],                    valign: 'bottom' },
        },
        body: [
            [
                d.lShule   + ' :',  d.school,
                d.lTeacher + ' :',  d.teacher
            ],
            [
                d.lClass   + ' :',  (d.classVal + ' ' + d.stream).trim(),
                d.lSubject + ' :',  d.subject
            ],
            [
                d.lTime    + ' :',  d.time,
                d.lDate    + ' :',  d.date
        ...
        didDrawCell: function (data) {
            // Dotted underline on value columns (1 and 3)
            if (
                data.section === 'body' &&
    ...
    // ─────────────────────────────────────────────
    // 3. STUDENTS TABLE  —  class="student-tb"
    //    Matches exactly:
    //    row1: colspan=6 heading
    //    row2: registered(3) | present(3)
    //    row3: girls/boys/total | girls/boys/total
    //    row4: values
    // ─────────────────────────────────────────────
    doc.autoTable({
        startY: y,
        ...
        head: [
            ...
            // Row 2 — registered / present
            [
                { content: d.lSajiliwa.toUpperCase(), colSpan: 3, styles: { fontStyle:'bold', fillColor: WHITE, textColor: BLACK } },
                { content: d.lFika.toUpperCase(),     colSpan: 3, styles: { fontStyle:'bold', fillColor: WHITE, textColor: BLACK } },
            ],
            // Row 3 — column labels
            [
                { content: d.lGirls.toUpperCase(),  styles: { fillColor: WHITE, textColor: BLACK } },
                { content: d.lBoys.toUpperCase(),   styles: { fillColor: WHITE, textColor: BLACK } },
                { content: d.lTotal.toUpperCase(),  styles: { fillColor: WHITE, textColor: BLACK } },
                { content: d.lGirls1.toUpperCase(), styles: { fillColor: WHITE, textColor: BLACK } },
                { content: d.lBoys1.toUpperCase(),  styles: { fillColor: WHITE, textColor: BLACK } },
                { content: d.lTotal1.toUpperCase(), styles: { fillColor: WHITE, textColor: BLACK } },
            ],
        ],
        body: [
            // Row 4 — values
    ...
    // ─────────────────────────────────────────────
    // 4. SYLLABUS TABLE  —  class="sylabus-tb"
    //    2-column: bold label | value with solid bottom border
    //    No outer borders on cells
    // ─────────────────────────────────────────────
    const syllabusRows = [
        [ d.lUjuzi    + ' :', d.sylbsMnObj ],
        [ d.lLengoKuu + ' :', d.sylbsCmp   ],
        [ d.lMadaNdogo+ ' :', d.sylbsSbtp  ],
        [ d.lMahususi + ' :', d.sylbsSpobj ],
        [ d.lNukuu    + ' :', d.sylbsMts   ],
        [ d.lRejea    + ' :', d.sylbsRfrs  ],
    ];
    ...
    doc.autoTable({
        ...
        styles: {
            fontSize: 8,
            cellPadding: { top: 1.2, bottom: 1.8, left: 1, right: 1 },
            textColor: BLACK,
            lineWidth: 0,
        },
        columnStyles: {
            0: { fontStyle: 'bold', cellWidth: 40,       textColor: BLACK, valign: 'bottom' },
            1: { cellWidth: CW - 40,                     valign: 'bottom' },
        },
        body: syllabusRows,
        didDrawCell: function (data) {
            // Solid bottom border on value column only
    ...
    // ─────────────────────────────────────────────
    // 5. PROCESS HEADING  —  <h3 id="kichwa-habari">
    // ─────────────────────────────────────────────
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.setTextColor(...BLACK);
    ...
    // ─────────────────────────────────────────────
    // 6. LESSON PROCESS TABLE  —  class="lesson-tb"
    //    Matches exactly:
    //    col1 stages | col2 time | col3 teaching | col4 learning | col5 assessment
    //    All 4 data rows share remaining page space equally
    // ─────────────────────────────────────────────
    const processColW = [27, 12, 52, 52, CW - 27 - 12 - 52 - 52]; // last col ~51mm

    // Remaining vertical space for process table
    const usedY     = y;
    const remarksH  = 10; // mm reserved for remarks at bottom
    const tableH    = PH - MB - remarksH - usedY;
    const headH     = 8;  // approximate header row height
    const rowH      = (tableH - headH) / 4; // equal height for 4 rows
    ...
    doc.autoTable({
        ...
        columnStyles: {
            0: { cellWidth: processColW[0], fontStyle: 'bold', halign: 'left'   },
            1: { cellWidth: processColW[1], halign: 'center'                    },
            2: { cellWidth: processColW[2]                                      },
            3: { cellWidth: processColW[3]                                      },
            4: { cellWidth: processColW[4]                                      },
        },
        head: [[
        ...
        body: [
            [ d.lMwanzo,     d.introCont, d.intro1,    d.intro2,    d.intro3    ],
            [ d.lMpya,       d.newKnow,   d.new1,      d.new2,      d.new3      ],
            [ d.lKuimarisha, d.reinCont,  d.rein1,     d.rein2,     d.rein3     ],
            [ d.lTafakari,   d.refleCont, d.reflect1,  d.reflect2,  d.reflect3  ],
        ],
        didParseCell: function (data) {
            // Force all body rows to share available height equally
    ...
    // ─────────────────────────────────────────────
    // 7. REMARKS  —  class="table-footer tb-footer"
    //    <p><b>REMARKS :</b> text</p>
    // ─────────────────────────────────────────────
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    doc.setTextColor(...BLACK);

    const remarkLabel = (d.lMaoni + ' :').toUpperCase();
    doc.text(remarkLabel, ML, y + 4);
...
function showLessonNotification(message, type = "info", duration = 3500) {
    _notifQueue.push({ message, type, duration });
    if (!_notifRunning) _processNotifQueue();
}

function _processNotifQueue() {
    if (_notifQueue.length === 0) { _notifRunning = false; return; }
    _notifRunning = true;

    const { message, type, duration } = _notifQueue.shift();

    const cfg = {
        success: { icon: 'bi-check-circle-fill',        accent: '#42b72a', bg: '#f0faf0', text: '#1a6b1a', border: '#42b72a' },
        error:   { icon: 'bi-x-circle-fill',            accent: '#e4405f', bg: '#fff0f3', text: '#8b1a2e', border: '#e4405f' },
        warning: { icon: 'bi-exclamation-triangle-fill', accent: '#f7b928', bg: '#fffbf0', text: '#7a5500', border: '#f7b928' },
        info:    { icon: 'bi-info-circle-fill',          accent: '#1877f2', bg: '#f0f5ff', text: '#0d3d8a', border: '#1877f2' }
    };
    const c = cfg[type] || cfg.info;

    // Account for sticky header: leave 70px gap on mobile, 20px on desktop
    const headerGap = window.innerWidth < 768 ? 70 : 20;
    const existing = document.querySelectorAll('.lesson-notification');
    let topOffset = headerGap;
    existing.forEach(n => { topOffset += n.offsetHeight + 6; });

    const n = document.createElement('div');
    n.className = `lesson-notification ln-${type}`;
    n.style.top = topOffset + 'px';
    n.innerHTML = `
        <div class="ln-accent" style="background:${c.accent}"></div>
        <div class="ln-icon" style="color:${c.accent}"><i class="bi ${c.icon}"></i></div>
        <span class="ln-msg">${message}</span>
        <button class="ln-close" title="Dismiss"><i class="bi bi-x-lg"></i></button>
    `;

    document.body.appendChild(n);
    requestAnimationFrame(() => n.classList.add('ln-show'));

    function dismiss() {
        n.classList.add('ln-hide');
        setTimeout(() => {
            n.remove();
            _restack();
            setTimeout(_processNotifQueue, 120);
        }, 280);
    }

    n.querySelector('.ln-close').addEventListener('click', dismiss);

    const timer = setTimeout(dismiss, duration);
    n.addEventListener('click', () => { clearTimeout(timer); dismiss(); }, { once: true });
}

function _restack() {
    const headerGap = window.innerWidth < 768 ? 70 : 20;
    let top = headerGap;
    document.querySelectorAll('.lesson-notification').forEach(n => {
        n.style.transition = 'top 0.25s ease';
        n.style.top = top + 'px';
        top += n.offsetHeight + 6;
    });
}
...
function showStepNotification(step, total, stepName) {
    const existing = document.getElementById('stepProgressNotif');
    if (existing) {
        existing.classList.add('spn-hide');
        setTimeout(() => existing.remove(), 250);
    }

    const dots = Array.from({ length: total }, (_, i) =>
        `<span class="spn-dot${i < step ? ' spn-dot-done' : i === step - 1 ? ' spn-dot-active' : ''}"></span>`
    ).join('');

    const n = document.createElement('div');
    n.id = 'stepProgressNotif';
    n.className = 'spn-wrap';
    n.innerHTML = `
        <div class="spn-left">
            <div class="spn-badge">
            ...
            <div class="spn-name">${stepName}</div>
            <div class="spn-dots">${dots}</div>
        </div>
        <button class="spn-close" title="Dismiss"><i class="bi bi-x-lg"></i></button>
    `;
    document.body.appendChild(n);
    requestAnimationFrame(() => n.classList.add('spn-show'));

    function dismiss() {
        n.classList.add('spn-hide');
        setTimeout(() => n.remove(), 280);
    }

    n.querySelector('.spn-close').addEventListener('click', dismiss);
...
function addEssentialStyles() {
    ...
    style.textContent = `
        ...
        /* ── Step Progress Notification (bottom-right) ── */
        .spn-wrap {
            position: fixed;
            bottom: 20px;
            right: 16px;
        ...
        .spn-wrap.spn-show { opacity: 1; transform: translateY(0); }
        .spn-wrap.spn-hide { opacity: 0; transform: translateY(14px); }
        .spn-left { flex: 1; min-width: 0; }
        .spn-badge {
            display: inline-flex; align-items: center; gap: 4px;
            background: rgba(66,183,42,0.15); border: 1px solid rgba(66,183,42,0.3);
            color: #42b72a; font-size: 0.62rem; font-weight: 800;
            padding: 2px 7px; text-transform: uppercase; letter-spacing: 0.05em;
            margin-bottom: 4px;
        }
        .spn-badge i { font-size: 0.7rem; }
        .spn-name {
            color: #fff; font-size: 0.8rem; font-weight: 700;
            margin-bottom: 6px; white-space: nowrap;
            overflow: hidden; text-overflow: ellipsis;
        }
        .spn-dots { display: flex; gap: 4px; align-items: center; }
        .spn-dot { width: 6px; height: 6px; background: rgba(255,255,255,0.18); display: block; }
        .spn-dot-done   { background: #42b72a; }
        .spn-dot-active { background: #1877f2; width: 13px; }
        .spn-close {
            background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.1);
            color: rgba(255,255,255,0.45); width: 20px; height: 20px;
            display: flex; align-items: center; justify-content: center;
            cursor: pointer; font-size: 0.58rem; flex-shrink: 0;
            align-self: flex-start; transition: none;
        }
        ...
        @media (max-width: 768px) {
            ...
            .spn-badge  { font-size: 0.56rem; padding: 1px 5px; margin-bottom: 2px; }
            .spn-name   { font-size: 0.7rem; margin-bottom: 4px; }
            .spn-dot    { width: 5px; height: 5px; }
            .spn-dot-active { width: 10px; }
            .spn-dots   { gap: 3px; }
            .spn-close  { width: 18px; height: 18px; font-size: 0.52rem; }
        }

        /* ── Regular Notifications (top-right, white card) ── */
        .lesson-notification {
            position: fixed;
            right: 16px;
            top: 20px;
        ...
        .lesson-notification.ln-show { opacity: 1; transform: translateX(0); }
        .lesson-notification.ln-hide { opacity: 0; transform: translateX(22px); }
        .ln-accent { width: 4px; align-self: stretch; flex-shrink: 0; }
        .ln-icon   { font-size: 0.95rem; flex-shrink: 0; display: flex; align-items: center; }
        .ln-msg    { flex: 1; font-size: 0.8rem; font-weight: 600; color: #1c1e21; line-height: 1.4; }
        .ln-close  {
        ...
        @media (max-width: 768px) {
            ...
            .ln-msg   { font-size: 0.74rem; }
            .ln-icon  { font-size: 0.85rem; }
            .ln-close { width: 18px; height: 18px; font-size: 0.6rem; }
        }
...
function addPdfLoaderStyles() {
    ...
    style.textContent = `
        ...
        .pdf-loader-card {
            ...
            animation: plSlideUp 0.35s cubic-bezier(0.22,1,0.36,1) both;
        }
        @keyframes plSlideUp {
        ...
        .pl-bar {
            ...
            width: 20%;
            animation: plBar 2.5s ease-in-out infinite alternate;
        }
        @keyframes plBar {
        ...
        .pl-tip-dot.active { background: #1877f2; width: 12px; }

        @media (max-width: 480px) {
            .pdf-loader-card { padding: 24px 18px 18px; }
            .pl-ring-wrap { width: 64px; height: 64px; margin-bottom: 16px; }
            .pl-icon { font-size: 1.2rem; }
            .pl-title { font-size: 0.9rem; }
            .pl-tip-text { font-size: 0.72rem; }
        }
    `;
    document.head.appendChild(style);
}
...
const _plTips = [
    { icon: 'bi-lightning-charge-fill', text: 'Your lesson plan is saved automatically — no data is lost even if you close the browser.' },
    { icon: 'bi-journal-check',          text: 'Visit the Summary to track all lesson plans across classes and subjects in one place.' },
    { icon: 'bi-shield-check',           text: 'All your data stays on your device — nothing is sent to external servers.' },
    { icon: 'bi-file-earmark-pdf',       text: 'Download a backup copy anytime from the lesson plan view — no internet needed.' },
    { icon: 'bi-calendar2-check',        text: 'Each lesson plan records the date, class and stream — great for term reports.' },
];

function _buildLoaderHTML() {
    const dots = _plTips.map((_, i) =>
        `<span class="pl-tip-dot${i === 0 ? ' active' : ''}"></span>`
    ).join('');
    return `
        <div id="pdfLoader" class="pdf-loader-overlay">
            <div class="pdf-loader-card">
                <div class="pl-ring-wrap">
                    ...
                    <div class="pl-icon"><i class="bi bi-file-earmark-pdf"></i></div>
                </div>
                <h3 class="pl-title">Generating Your PDF</h3>
                <p class="pl-status" id="plStatus">Preparing content…</p>
                <div class="pl-bar-wrap"><div class="pl-bar"></div></div>
                <div class="pl-tip-wrap">
                    <div class="pl-tip-label"><i class="bi bi-lightbulb-fill"></i> Quick Tip</div>
                    <p class="pl-tip-text" id="plTipText">${_plTips[0].text}</p>
                </div>
                <div class="pl-tip-dots" id="plTipDots">${dots}</div>
            </div>
        </div>
    `;
}

let _plTipTimer = null;
function _startTipCycle() {
    let idx = 0;
    _plTipTimer = setInterval(() => {
        idx = (idx + 1) % _plTips.length;
        const el = document.getElementById('plTipText');
        const dots = document.querySelectorAll('.pl-tip-dot');
        if (el) { el.style.opacity = 0; setTimeout(() => { el.textContent = _plTips[idx].text; el.style.opacity = 1; el.style.transition = 'opacity 0.4s'; }, 300); }
        dots.forEach((d, i) => { d.classList.toggle('active', i === idx); });
    }, 3000);
}

const _plStatuses = ['Preparing content…', 'Processing layout…', 'Rendering pages…', 'Finalizing PDF…'];

function showPdfLoader() {
    addPdfLoaderStyles();
    document.body.insertAdjacentHTML('beforeend', _buildLoaderHTML());
    document.body.style.overflow = 'hidden';
    _startTipCycle();
}

function hidePdfLoader() {
    clearInterval(_plTipTimer);
    _plTipTimer = null;
...
function markTrackerRecordDownloaded() {
    try {
        const subject        = document.getElementById('subject')?.value || '';
        const classes        = document.getElementById('classes')?.value || '';
        const specificLesson = document.getElementById('specificLesson')?.value || '';
        ...
        if (idx !== -1) {
            records[idx].downloaded    = true;
            records[idx].downloadCount = (records[idx].downloadCount || 0) + 1;
            records[idx].lastDownloadedAt = new Date().toISOString();
            localStorage.setItem('lessonPlanTracker', JSON.stringify(records));
        }
    } catch (e) {}
}
...
function filterTrackerRecords(records, filter) {
    if (filter === 'downloaded')     return records.filter(r => r.downloaded);
    if (filter === 'not_downloaded') return records.filter(r => !r.downloaded);
    const now = new Date();
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        ...
        html += `
            <div class="tracker-class-group">
                ...
                <div class="tracker-plans-list">
                    ${plans.map((p, i) => {
                        const dlBadge = p.downloaded
                            ? `<span class="tpi-dl-badge tpi-dl-yes"><i class="bi bi-download"></i> Downloaded${p.downloadCount > 1 ? ` (${p.downloadCount}×)` : ''}</span>`
                            : `<span class="tpi-dl-badge tpi-dl-no"><i class="bi bi-hourglass-split"></i> Not Downloaded</span>`;
                        const dlTime = p.lastDownloadedAt
                            ? `<span class="tpi-dl-time"><i class="bi bi-clock-history"></i> Last download: ${formatExactTime(p.lastDownloadedAt)}</span>`
                            : '';
                        return `
...
function showTrackerModal(activeFilter) {
    ...
    const downloaded     = allRecords.filter(r => r.downloaded).length;
    ...
    const filters = [
        ...
        { key: 'downloaded',    label: '<i class="bi bi-download"></i> Downloaded' },
        { key: 'not_downloaded',label: '<i class="bi bi-clock"></i> Not Downloaded' }
    ];
        ...
        <div id="trackerModal" class="tracker-modal-overlay">
            <div class="tracker-modal">
                ...
                <div class="tracker-stats-bar">
                    ...
                    <div class="tracker-stat tstat-dl">
                        <span class="tracker-stat-num tstat-dl-num">${downloaded}</span>
                        <span class="tracker-stat-label">Downloaded</span>
                    </div>
                    <div class="tracker-stat tstat-ndl">
                        <span class="tracker-stat-num tstat-ndl-num">${notDownloaded}</span>
                        <span class="tracker-stat-label">Not Downloaded</span>
                    </div>
                    <div class="tracker-stat">
                ...
                <div class="tracker-modal-footer">
                    <button class="tmf-btn tmf-danger" onclick="clearTrackerData()">
                        <i class="bi bi-trash3"></i> Clear All History
                    </button>
                    <button class="tmf-btn tmf-close" onclick="document.getElementById('trackerModal').remove(); document.body.style.overflow='auto';">
...
function addTrackerStyles() {
    ...
    s.textContent = `
        ...
        .tpi-dl-yes {
            background: #e8f9e8; color: #2a7a2a;
            border: 1px solid #b3e6b3;
        }
        .tpi-dl-no {
            background: #fff4e5; color: #7a4f00;
        ...
        .tstat-dl .tracker-stat-num { color: #42b72a; }
        .tstat-ndl .tracker-stat-num { color: #f7b928; }
        .tpi-updated {
        ...
        .tracker-modal-footer {
            ...
            display: flex; justify-content: flex-end; gap: 0; flex-shrink: 0;
        }
        .tmf-btn {
            display: inline-flex; align-items: center; gap: 7px;
            padding: 11px 20px; border: none; cursor: pointer;
            font-size: 0.875rem; font-weight: 700; transition: none;
        }
        .tmf-danger {
            background: #fdeaee; color: #e4405f;
            border-right: 1px solid #f5c2cc;
        }
        .tmf-close {
        ...
        @media (max-width: 768px) {
            ...
            .tracker-modal-footer { padding: 12px 16px; flex-direction: column; gap: 0; }
            .tmf-btn { width: 100%; justify-content: center; padding: 11px 14px; font-size: 0.875rem; }
            .tmf-danger { border-right: none; border-bottom: 1px solid #f5c2cc; }
        }
        @media (max-width: 480px) {
            ...
            .tmf-btn { padding: 10px 12px; font-size: 0.84rem; }
        }
    `;
    document.head.appendChild(s);
}
function initDateTimeWidget() {
    ...
    const isHidden = isMobile() ? localStorage.getItem('ldtHidden') !== 'false' : false;

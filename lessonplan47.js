...
document.getElementById('subject').addEventListener('change', function() {
 ...
 const wrdImp1 = document.getElementById('wrdImp1');
  ...
  wrdImp2.innerHTML = '<option value="">Chagua neno la kutumia</option>';
  wrdImp1.innerHTML = '<option value="">Chagua neno la utangulizi</option>';
  } else {
    wrdImp2.innerHTML = '<option value="">Select word to use</option>';
    wrdImp1.innerHTML = '<option value="">Select word for introduction</option>';
  }
  let language = allowedSubjects.includes(selectedSubject) ? 'kiswahili' : 'english';
  
  const wordList = words[language].wrdImp2;
...
const wordList1 = words[language].wrdImp1;
  wordList1.forEach(function(word) {
    const option = document.createElement('option');
    option.value = word;
    option.textContent = word;

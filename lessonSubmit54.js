...
class LessonPlanForm {
    ...
    saveFormData() {
        ...
        try {
            localStorage.setItem('lptz_nsc_formData', JSON.stringify(dataToSave));
        } catch (error) {
        }
    }
    loadSavedData() {
        try {
            const savedData = localStorage.getItem('lptz_nsc_formData');
            if (savedData) {
    ...
    clearSavedData() {
        try {
            localStorage.removeItem('lptz_nsc_formData');
        } catch (error) {
        }
    }
...
function getCurrentSavedPlan() {
    try {
        const stored = localStorage.getItem('lptz_nsc_currentPlan');
        if (stored) {
            return JSON.parse(stored);
        }
        const formStored = localStorage.getItem('lptz_nsc_formData');
        if (formStored) {
...
function saveCurrentPlan(formData, isAutoSave = false) {
    try {
        ...
        localStorage.setItem('lptz_nsc_currentPlan', JSON.stringify(planData));
        return true;
    } catch (error) {
...
function clearCurrentSavedPlan() {
    try {
        localStorage.removeItem('lptz_nsc_currentPlan');
        localStorage.removeItem('lptz_nsc_formData');
    } catch (error) {
    }
}
...
function migrateLocalStorageKeys() {
    const migrations = [
        { old: 'lessonPlanFormData',  newer: 'lptz_nsc_formData'    },
        { old: 'currentLessonPlan',   newer: 'lptz_nsc_currentPlan'  },
        { old: 'lessonPlanTracker',   newer: 'lptz_nsc_tracker'      },
        { old: 'ldtHidden',           newer: 'lptz_nsc_clockHidden'  }
    ];
    migrations.forEach(({ old, newer }) => {
        try {
            const existing = localStorage.getItem(old);
            if (existing !== null && localStorage.getItem(newer) === null) {
                localStorage.setItem(newer, existing);
...
function recordLessonPlanToTracker() {
    try {
        ...
        localStorage.setItem('lptz_nsc_tracker', JSON.stringify(existing));
    } catch (e) {}
}
function getTrackerData() {
    try {
        const stored = localStorage.getItem('lptz_nsc_tracker');
...
function markTrackerRecordDownloaded() {
    try {
        ...
        if (idx !== -1) {
            ...
            localStorage.setItem('lptz_nsc_tracker', JSON.stringify(records));
        }
    } catch (e) {}
}
...
function clearTrackerData() {
    showConfirmationModal({
        ...
        confirmClass: "modal-btn-danger",
        onConfirm: function() {
            localStorage.removeItem('lptz_nsc_tracker');
...
function initDateTimeWidget() {
    ...
    const isHidden = isMobile() ? localStorage.getItem('lptz_nsc_clockHidden') !== 'false' : false;
    ...
    document.getElementById('ldtHideBtn').addEventListener('click', function() {
        ...
        localStorage.setItem('lptz_nsc_clockHidden', 'true');
        document.getElementById('lpDateTimeWidget').style.pointerEvents = 'none';
    ...
    document.getElementById('ldtShowBtn').addEventListener('click', function() {
        ...
        localStorage.setItem('lptz_nsc_clockHidden', 'false');
    ...
    window.addEventListener('resize', function() {
        if (!isMobile()) {
            ...
            document.getElementById('lpDateTimeWidget').style.pointerEvents = 'auto';
            document.body.style.paddingBottom = '';
        } else if (localStorage.getItem('lptz_nsc_clockHidden') === 'true') {
            document.getElementById('ldtFull').style.display = 'none';
            const showBtn = document.getElementById('ldtShowBtn');
            showBtn.style.display = 'flex';
            document.getElementById('lpDateTimeWidget').style.pointerEvents = 'none';

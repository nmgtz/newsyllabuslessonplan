...
/*
 * ═══════════════════════════════════════════════════════════════════════════
 * SHARED SUBJECT-LANGUAGE DETECTION
 * ───────────────────────────────────────────────────────────────────────────
 * Single source of truth for "which subjects are taught in Swahili".
 * Previously this same array was copy-pasted in five different places
 * (formatReference, getEducationLevel, insertEducationLevel,
 * buildIntelligentNounList, updateContent). Any edit to one copy could
 * silently diverge from the others. Now everything calls isSwahiliSubject().
 * ═══════════════════════════════════════════════════════════════════════════
 */
const SWAHILI_SUBJECTS = [
    ...
    "jiografia na mazingira", "hisabati", "sayansi", "maarifa ya jamii",
    "stadi za kazi", "uraia na maadili"
];

function isSwahiliSubject(subject) {
    return !!subject && SWAHILI_SUBJECTS.includes(subject.toLowerCase());
}
...
/*
 * FIX (previously): this function passed activity.original (the FULL raw
 * lesson text, e.g. "Importance of Life Skills in Daily Life") into
 * buildContextualLabel. Since the connector phrase already encodes the
 * concept word (e.g. "zinazobainisha thamani ya" = "showing the importance
 * of"), passing the full original text caused the concept to appear twice:
 *   "Picha zinazobainisha thamani ya Importance of Life Skills in Daily Life"
 * Now we pass activity.coreSubject, which parseActivity() has already
 * stripped down to just the subject (e.g. "Life Skills in Daily Life"),
 * so the concept word is stated exactly once.
 */
function buildTLMResourcesPhrase(tlmString, specificLesson, isSwahili) {
    ...
    return capitalizeFirst(
        buildContextualLabel(options[0], activity.conceptType, activity.coreSubject, isSwahili)
    );
}
...
function toGerundWord(v) {
    if (v.length > 1 && v.endsWith('e') && !v.endsWith('ee')) return v.slice(0, -1) + 'ing';
    return v + 'ing';
}
function toGerundPhrase(phrase) {
    return phrase.split(' and ').map(toGerundWord).join(' and ');
}
const EN_IRREGULAR_PP = { give: 'given', write: 'written' };
function toPastParticiple(v) {
    if (EN_IRREGULAR_PP[v]) return EN_IRREGULAR_PP[v];
...
const EN_ASSESS_DATA = {
    meaning_importance:  { object: "the meaning and importance of {S}",       plural: true,  verbs: ["state", "explain", "outline"] },
    meaning:             { object: "the meaning of {S}",                     plural: false, verbs: ["explain", "state", "describe"] },
    importance:          { object: "the importance of {S}",                  plural: false, verbs: ["state", "identify", "explain"] },
    effects:             { object: "the effects of {S}",                     plural: true,  verbs: ["identify", "describe", "explain"] },
    causes:              { object: "the causes of {S}",                      plural: true,  verbs: ["identify", "explain", "describe"] },
    advantages:          { object: "the advantages of {S}",                  plural: true,  verbs: ["list", "explain", "identify"] },
    disadvantages:       { object: "the disadvantages of {S}",               plural: true,  verbs: ["identify", "describe", "list"] },
    adv_disadv:          { object: "the advantages and disadvantages of {S}", plural: true,  verbs: ["compare", "evaluate", "assess"] },
    types:               { object: "the types of {S}",                      plural: true,  verbs: ["identify", "classify", "describe"] },
    characteristics:     { object: "the characteristics of {S}",            plural: true,  verbs: ["list", "describe", "identify"] },
    steps:               { object: "the steps of {S}",                      plural: true,  verbs: ["sequence", "describe", "outline"] },
    examples:            { object: "examples of {S}",                       plural: true,  verbs: ["give", "identify", "provide"] },
    definition:          { object: "a definition of {S}",                   plural: false, verbs: ["state", "write", "give"] },
    uses:                { object: "the uses of {S}",                       plural: true,  verbs: ["identify", "list", "describe"] },
    functions:           { object: "the functions of {S}",                  plural: true,  verbs: ["explain", "list", "identify"] },
    components:          { object: "the components of {S}",                 plural: true,  verbs: ["identify", "name", "describe"] },
    solutions:           { object: "solutions to {S}",                      plural: true,  verbs: ["propose", "identify", "suggest"] },
    generic:             { object: "key ideas about {S}",                   plural: true,  verbs: ["explain", "describe", "discuss"] },
};
 
const EN_TEMPLATES = [
    (v, o, plural, m) => `${capitalizeFirst(o)} ${plural ? 'are' : 'is'} ${toPastParticiple(v)}${m}`,
    (v, o, plural, m) => `${capitalizeFirst(o)} ${plural ? 'are' : 'is'} ${toPastParticiple(v)}${m} by each student`,
    (v, o, plural, m) => `Each student has ${toPastParticiple(v)} ${decapitalize(o)}${m}`,
    (v, o, plural, m) => `Each student has shown the skill of ${toGerundPhrase(v)} ${decapitalize(o)}${m}`,
    (v, o, plural, m) => `Each student has managed to ${v} ${decapitalize(o)}${m}`,
    (v, o, plural, m) => `By the end of the activity, ${decapitalize(o)} ${plural ? 'have' : 'has'} been ${toPastParticiple(v)}${m}`,
    (v, o, plural, m) => `Each student has come to ${v} ${decapitalize(o)}${m}`,
    (v, o, plural, m) => `Each student has gone on to ${v} ${decapitalize(o)}${m}`,
];
const EN_REALIZATION_MODS = [' independently', ' on their own', ' individually'];
 
function buildEnglishAssessment(conceptType, coreSubject, original, stage) {
    const key = (conceptType && EN_ASSESS_DATA[conceptType]) ? conceptType : 'generic';
...
const SW_ASSESS_DATA = {
    meaning_importance:  { object: "maana na umuhimu wa {S}", verbs: ["eleza", "taja", "onyesha"] },
    meaning:             { object: "maana ya {S}",            verbs: ["eleza", "fafanua", "taja"] },
    importance:          { object: "umuhimu wa {S}",          verbs: ["taja", "bainisha", "eleza"] },
    effects:             { object: "athara za {S}",           verbs: ["bainisha", "taja", "eleza"] },
    causes:              { object: "sababu za {S}",           verbs: ["bainisha", "orodhesha", "eleza"] },
    advantages:          { object: "faida za {S}",            verbs: ["orodhesha", "eleza", "bainisha"] },
    disadvantages:       { object: "hasara za {S}",           verbs: ["bainisha", "taja", "orodhesha"] },
    adv_disadv:          { object: "faida na hasara za {S}",  verbs: ["linganisha", "tathmini", "bainisha"] },
    types:               { object: "aina za {S}",             verbs: ["tambua", "bainisha", "orodhesha"] },
    characteristics:     { object: "sifa za {S}",             verbs: ["orodhesha", "eleza", "bainisha"] },
    steps:               { object: "hatua za {S}",            verbs: ["panga", "bainisha", "eleza"] },
    examples:            { object: "mifano ya {S}",           verbs: ["toa", "tambua", "onyesha"] },
    definition:          { object: "ufafanuzi wa {S}",        verbs: ["andika", "taja", "eleza"] },
    uses:                { object: "matumizi ya {S}",         verbs: ["bainisha", "orodhesha", "taja"] },
    functions:           { object: "kazi za {S}",             verbs: ["eleza", "orodhesha", "bainisha"] },
    components:          { object: "sehemu za {S}",           verbs: ["taja", "bainisha", "orodhesha"] },
    solutions:           { object: "suluhu za {S}",           verbs: ["pendekeza", "bainisha", "toa"] },
    generic:             { object: "dhana kuu za {S}",        verbs: ["eleza", "onyesha", "bainisha"] },
};
 
const SW_TEMPLATES = [
    (v, o, m) => `Kila mwanafunzi ame${v} ${o}${m}`,
    (v, o, m) => `Kila mwanafunzi ameonyesha uwezo wa ku${v} ${o}${m}`,
    (v, o, m) => `Ku${v} ${o} ni jambo ambalo kila mwanafunzi ameonyesha${m}`,
    (v, o, m) => `Kila mwanafunzi amefanikiwa ku${v} ${o}${m}`,
    (v, o, m) => `Baada ya shughuli, kila mwanafunzi ame${v} ${o}${m}`,
    (v, o, m) => `Kila mwanafunzi amethibitisha uelewa wake kwa ku${v} ${o}${m}`,
    (v, o, m) => `Kila mwanafunzi ameendelea ku${v} ${o}${m}`,
    (v, o, m) => `Kila mwanafunzi ameweza ku${v} ${o}${m}`,
];
const SW_REALIZATION_MODS = [' peke yake', ' kwa kujitegemea', ' bila msaada'];
 
function buildSwahiliAssessment(conceptType, coreSubject, original, stage) {
    const key = (conceptType && SW_ASSESS_DATA[conceptType]) ? conceptType : 'generic';
...
/*
 * FIX (previously): this function carried its own local copy of
 * allowedSubjects, duplicated in 4 other places in this file. Now uses
 * the single shared isSwahiliSubject() helper defined at the top.
 */
function formatReference(ref, subject) {
    if (!ref || ref.trim() === '') return '';
    const originalRef = ref.trim();
    ref = originalRef;
    const isSwahili = isSwahiliSubject(subject);
    ref = isSwahili
...
/*
 * FIX: uses shared isSwahiliSubject() helper instead of its own local
 * allowedSubjects array copy.
 */
...
function updateContent() {
    ...
    /*
     * FIX (previously): isSwahiliSubjectFlag used to be computed further
     * down in this function, AFTER the line that already tried to use a
     * (never-declared) variable called isSwahiliSubject — causing a
     * ReferenceError every time updateContent() ran. Now it's computed
     * here, right at the top, using the single shared helper, and reused
     * consistently by every line below that needs to know the subject's
     * language.
     */
    const isSwahiliSubjectFlag = isSwahiliSubject(subject);

...
var lessonplanSubmit = document.getElementById("submit-lesson-plan");
    ...
    const normalizedSubjects = allowedSubjects.map(subject => subject.toLowerCase());
    ...
    document.getElementById('subject').addEventListener('change', function () {
      const selectedSubject = this.value.trim().toLowerCase();
      const wrdImp2 = document.getElementById('wrdImp2');
      const wrdImp1 = document.getElementById('wrdImp1');

      const isKiswahili = normalizedSubjects.includes(selectedSubject);
      const language = isKiswahili ? 'kiswahili' : 'english';

      // Clear and set default options
      wrdImp2.innerHTML = `<option value="">${isKiswahili ? "Chagua neno la kutumia" : "Select word to use"}</option>`;
      wrdImp1.innerHTML = `<option value="">${isKiswahili ? "Chagua neno la utangulizi" : "Select word for introduction"}</option>`;

      // Add words for wrdImp2
      words[language].wrdImp2.forEach(word => {
        const option = document.createElement('option');
        option.value = word;
        option.textContent = word;
        wrdImp2.appendChild(option);
      });

      // Add words for wrdImp1
      words[language].wrdImp1.forEach(word => {
        const option = document.createElement('option');
        option.value = word;
        option.textContent = word;
        wrdImp1.appendChild(option);
      });
    });

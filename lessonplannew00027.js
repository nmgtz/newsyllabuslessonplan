/**
 * lessonPlanPhrases.js  — Advanced Lesson Plan Phrase Generator (v3)
 * ===================================================================
 * Fixes applied over v2:
 *  1. TLM descriptions are now CONTEXTUAL — the label used in phrases
 *     dynamically reflects the specific objective, e.g.
 *     "a video demonstrating the importance of biology"
 *     instead of the static word "a video clip".
 *  2. Assessments NEVER mention groups — always framed at individual
 *     student level (no "Groups correctly…" anywhere).
 *  3. Phrase pools are significantly expanded and shuffled so repeated
 *     runs of the same lesson produce varied, non-static output.
 */

// ─────────────────────────────────────────────────────────────────────────────
// 1. SWAHILI TIME CONVERSION
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. TLM → CONTEXTUAL STIMULUS MAPPER
//
// KEY FIX: buildStimulusLabel() generates a CONTEXTUAL description of the TLM
// that incorporates the specific objective, e.g.:
//   "a video demonstrating the importance of biology"
//   "pictures showing the types of soil"
//   "a chart illustrating the effects of deforestation"
// This replaces the static labels like "a video clip", "pictures", etc.
// ─────────────────────────────────────────────────────────────────────────────
const TLM_MAP = {
...
// ─────────────────────────────────────────────────────────────────────────────
// 7. ASSESSMENT BUILDERS
//
// KEY FIX: ALL assessments are now framed at INDIVIDUAL STUDENT level.
// No "Groups correctly…" language anywhere. Stages determine the depth
// of the assessment criterion, not whether it targets groups or individuals.
// ─────────────────────────────────────────────────────────────────────────────
...
function generateEnglishPhrases(verb, specificLesson, isContinue, tlmString) {
    ...
    if (isContinue) {
        const introTeacher = [
            `${capitalizeFirst(stimulus.verb)} ${stimulus.label} from the previous lesson and ask students questions to check their understanding of ${activity.original}`,
            `Show resources related to ${activity.original} used in the previous lesson and pose review questions to the class`,
            `Present ${stimulus.label} from the last lesson and facilitate a brief recall discussion on ${activity.original}`,
            `Display ${stimulus.label} from the previous period and ask guiding questions to consolidate students' understanding of ${activity.original}`,
            `Use ${stimulus.label} from the previous lesson to prompt students to recall what they learned about ${activity.original}`,
        ];
        const introStudent = [
            `Observe the ${stimulus.baseLabel} and respond to questions about ${activity.original} from the previous lesson`,
            `Examine the resources and answer review questions on ${activity.original}`,
            `Participate in a recall discussion about ${activity.original} based on the previous lesson`,
            `Engage with the materials and answer questions based on prior knowledge of ${activity.original}`,
            `Review the ${stimulus.baseLabel} and share what they remember about ${activity.original}`,
        ];
        phrases.intro1 = pick(introTeacher);
        phrases.intro2 = pick(introStudent);
        phrases.intro3 = pick([
            `Review questions about ${activity.original} from the previous lesson are correctly answered`,
            `Each student correctly answers recall questions about ${activity.original}`,
            `Prior knowledge of ${activity.original} is accurately recalled`,
        ]);
    } else {
        const introTeacher = [
...
function generateSwahiliPhrases(verb, specificLesson, isContinue, tlmString) {
    ...
    if (isContinue) {
        const introTeacher = [
            `${capitalizeFirst(stimulus.verb)} ${stimulus.label} kutoka somo lililopita na kuuliza maswali ya ukaguzi wa uelewa wa ${activity.original}`,
            `Kuwasilisha vifaa vinavyohusiana na ${activity.original} vilivyotumika somo la awali na kuwauliza wanafunzi maswali ya marudio`,
            `Kutumia ${stimulus.label} kutoka kipindi kilichopita na kuwauliza wanafunzi maswali ya kukagua uelewa wa ${activity.original}`,
            `Kuonyesha ${stimulus.label} kutoka somo lililotangulia na kufanya majadiliano ya marudio ya ${activity.original}`,
            `Kutumia ${stimulus.label} kutoka somo lililopita kuwakumbusha wanafunzi walichojifunza kuhusu ${activity.original}`,
        ];
        const introStudent = [
            `Kujibu maswali ya marudio kuhusu ${activity.original} kwa kutazama ${stimulus.baseLabel} za somo lililopita`,
            `Kuchunguza vifaa na kujibu maswali ya marudio kuhusu ${activity.original}`,
            `Kushiriki katika majadiliano ya ukumbusho kuhusu ${activity.original} kutokana na somo lililotangulia`,
            `Kujishughulisha na nyenzo na kujibu maswali kwa kutumia maarifa yao ya awali ya ${activity.original}`,
            `Kupitia ${stimulus.baseLabel} na kushiriki wanachokikumbuka kuhusu ${activity.original}`,
        ];
        phrases.intro1 = pick(introTeacher);
        phrases.intro2 = pick(introStudent);
        phrases.intro3 = pick([
            `Maswali ya marudio kuhusu ${activity.original} yamejibiwa kwa usahihi`,
            `Kila mwanafunzi anajibu kwa usahihi maswali ya marudio ya ${activity.original}`,
            `Maarifa ya awali ya ${activity.original} yamerejeshwa kwa usahihi`,
        ]);
    } else {
        const introTeacher = [
...
// ─────────────────────────────────────────────────────────────────────────────
// 13. updateContent — now passes conceptType + specificLesson to getStimulusFromTLM
// ─────────────────────────────────────────────────────────────────────────────

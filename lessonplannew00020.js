...
function formatReference(ref, subject) {
    ...
    if (yearMatch) {
        ...
        yearPart = '(' + detectedYear + ')';
        afterYear = restPart.substring(yearMatch[0].length).trim();
        
        // Remove any leading comma or period after year extraction
        afterYear = afterYear.replace(/^[,.\s]+/, '').trim();
    }
    
    // **HARDCODED FALLBACK: Remove common publishers at the end**
    ...
    publishersToRemove.forEach(publisher => {
        const patterns = [
            new RegExp(',\\s*' + publisher.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\.?$', 'gi'),
            new RegExp('\\.\\s*' + publisher.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\.?$', 'gi')
        ];
        patterns.forEach(pattern => {
            afterYear = afterYear.replace(pattern, '');
        });
    });
    ...
    if (titlePart) {
        // Add education level before class name if detected
        var finalTitle = titlePart;
        
        if (educationLevel) {
            // Insert education level before the class name
            finalTitle = insertEducationLevel(titlePart, educationLevel, subject);
        }
        
        // Add title without italics
        formatted += ' ' + finalTitle + '.';
    }
    
    // Add publisher at the end (same as author for TIE books)
...
function getEducationLevel(ref, subject) {
    ...
    // Primary level patterns (Swahili)
    const primarySwahili = [
        'darasa la awali', 'darasa la kwanza', 'darasa la pili', 'darasa la tatu',
        'darasa la nne', 'darasa la tano', 'darasa la sita', 'darasa la saba'
    ];
    
    // Primary level patterns (English)
    const primaryEnglish = [
        ...
        'pre-primary', 'pre primary', 'class i', 'class ii', 'class iii'
    ];
    
    // Secondary level patterns (Swahili)
    const secondarySwahili = [
        'kidato cha kwanza', 'kidato cha pili', 'kidato cha tatu', 'kidato cha nne'
    ];
    
    // Secondary level patterns (English)
    const secondaryEnglish = [
        'form one', 'form two', 'form three', 'form four',
        'form 1', 'form 2', 'form 3', 'form 4',
        'form i', 'form ii', 'form iii', 'form iv'
    ];
    
    // Advanced level patterns (Swahili)
    const advancedSwahili = [
        'kidato cha tano', 'kidato cha sita'
    ];
    
    // Advanced level patterns (English)
    const advancedEnglish = [
        'form five', 'form six', 'form 5', 'form 6',
        'form v', 'form vi', 'advanced level', 'a-level'
    ];
    
    // Check for Advanced level
    if (advancedSwahili.some(pattern => refLower.includes(pattern)) || 
        advancedEnglish.some(pattern => refLower.includes(pattern))) {
        return isSwahili ? 'kwa shule za sekondari ya juu' : 'for advanced secondary schools';
    }
    
    // Check for Secondary level
    if (secondarySwahili.some(pattern => refLower.includes(pattern)) || 
        secondaryEnglish.some(pattern => refLower.includes(pattern))) {
        return isSwahili ? 'kwa shule za sekondari' : 'for secondary schools';
    }
    
    // Check for Primary level
    if (primarySwahili.some(pattern => refLower.includes(pattern)) || 
        primaryEnglish.some(pattern => refLower.includes(pattern))) {
        return isSwahili ? 'kwa shule za msingi' : 'for primary schools';
    }
...
function insertEducationLevel(title, educationLevel, subject) {
    ...
    // **CHECK IF EDUCATION LEVEL ALREADY EXISTS IN TITLE**
    const titleLower = title.toLowerCase();
    
    // Check if any education level phrase already exists
    const educationLevelPhrases = [
        'kwa shule za msingi',
        'kwa shule za sekondari',
        'kwa shule za sekondari ya juu',
        'for primary schools',
        'for secondary schools',
        'for advanced secondary schools'
    ];
    
    // If education level already exists, don't add it again
    if (educationLevelPhrases.some(phrase => titleLower.includes(phrase))) {
        return title;
    }
    
    // Patterns to find where to insert education level
    if (isSwahili) {
        // For Swahili: Insert before "darasa la" or "kidato cha"
        if (titleLower.includes('darasa la') || titleLower.includes('kidato cha')) {
            // Find "Kitabu cha mwanafunzi" and insert level after it
            const pattern = /(kitabu cha mwanafunzi)\s+(darasa la|kidato cha)/i;
            if (pattern.test(title)) {
                return title.replace(pattern, `$1 ${educationLevel}, $2`);
            }
        }
    } else {
        // For English: Insert before "standard" or "form"
        if (titleLower.includes('standard ') || titleLower.includes('form ')) {
            // Find "Pupil's book" or "Student's book" and insert level after it
            const pattern = /(pupil'?s book|student'?s book)\s+(standard|form)/i;
            if (pattern.test(title)) {
                return title.replace(pattern, `$1 ${educationLevel}, $2`);
            }
        }
    ...
    // Fallback: append with comma after education level
...
function sentenceCaseWithProperNouns(str, originalRef, subject) {
    ...
    // Capitalize after punctuation (periods, exclamation, question marks)
    str = str.replace(/([.!?]\s+)([a-z])/g, function(match, punct, letter) {
        return punct + letter.toUpperCase();
    });
    
    // **FIX: Normalize colons (remove space before, ensure space after)**
    str = str.replace(/\s*:\s*/g, ': ');
    
    // **FIX: Add colon before subtitles and capitalize after**
    // Using non-capturing group (?:...) to avoid parameter confusion
    str = str.replace(/\b(?:kitabu cha)\b/gi, function(match, offset, fullStr) {
        const beforeMatch = fullStr.substring(0, offset).trim();
...
function buildIntelligentNounList(str, subject) {
    ...
    // Universal proper nouns (always capitalize)
    const universal = [
    ...
    // **CRITICAL: For Swahili subjects, DO NOT capitalize common Swahili words in titles**
    // Only capitalize genuinely proper nouns (religions, places, languages)
    const swahiliProperNouns = [
        'Kiislamu', 'Kiislam', 'Uislamu', 'Islam',
        'Kikristo', 'Ukristo',
        'Kiyahudi', 'Uyahudi'
    ];
    
    // English keywords (for English subjects)
    ...
    const keywords = isSwahiliSubject ? swahiliProperNouns : englishKeywords;
...
if (window.userSelectedReferenceYear) {
    formattedRef = applyUserYear(formattedRef, window.userSelectedReferenceYear);
}

...
function sentenceCaseWithProperNouns(str, originalRef, subject) {
    ...
    // This handles patterns like "main title subtitle" → "Main title: Subtitle"
    str = str.replace(/\b(kitabu cha)/gi, function(match) {
        // Check if there's text before this phrase that doesn't end with colon
        const beforeMatch = str.substring(0, str.toLowerCase().indexOf(match.toLowerCase()));
        if (beforeMatch && !beforeMatch.trim().endsWith(':')) {

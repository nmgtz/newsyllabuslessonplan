...
function buildAndDownloadPdf(filename, onSuccess) {
    ...
    doc.rect(ML, remarksBoxY, CW, remarkRowH);

    doc.setFont('helvetica','bold');
    doc.setFontSize(REMARKS_FS);
    doc.setTextColor(...BLACK);
    const remarkLabel = lbl(d.lMaoni);
    doc.text(remarkLabel, ML + 1.5, remarksBoxY + REMARKS_VPAD + REMARKS_LINE_H - 1);

    doc.setDrawColor(...BLACK);
    doc.setLineWidth(0.3);
    doc.line(ML + REMARKS_LABEL_W, remarksBoxY, ML + REMARKS_LABEL_W, remarksBoxY + remarkRowH);

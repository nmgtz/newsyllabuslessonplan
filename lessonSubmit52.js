...
lessonplanSubmit.addEventListener('click', function (mySb) {
    ...
    downloadBtn.addEventListener('click', function() {
        if (!_pdfDownloaded) {
            _pdfDownloaded = true;
            _doDownload();
            // Button changes to yellow only after user sees the success toast
            // and the download is confirmed — we delay slightly for UX
            setTimeout(_markAsDownloaded, 1200);
        } else {
            showConfirmationModal({
                ...
                onConfirm: function() { _doDownload(); markTrackerRecordDownloaded(); }
            });
        }
    });

...
// 📁 ENHANCED LESSON PLAN FOLDER SYSTEM WITH POP-UP & AUTO-SAVE
function initializeLessonPlanFolder() {
    ...
    // Create folder modal if it doesn't exist
...
// 📁 Create folder modal (pop-up view)
...
function displayFolderModal() {
    ...
    const emptyFolder = document.getElementById('modalEmptyFolder');
...
function setupAutoSaveSystem() {
    // Auto-save when form fields change (with debounce)
    let autoSaveTimeout;
    ...
    // Also auto-save when lesson content is updated
    if (typeof updateContent === 'function') {
        ...
        window.updateContent = function() {
            ...
            autoSaveTimeout = setTimeout(() => {
                performAutoSave();
            }, 3000); // Auto-save after content update
        };
    }
}

// 💾 Perform auto-save with smart title handling
function performAutoSave() {
    try {
        if (!hasMinimumRequiredFields()) {
            return; // Don't auto-save if minimum fields aren't filled
        }
        ...
        if (!lessonName || lessonName === 'Subject - Lesson ()') {
            return; // Don't save with default/empty name
        }
        ...
        if (existingPlan) {
            // Update existing plan (auto-save doesn't ask for confirmation)
            updateExistingLessonPlan(lessonName, formData, true);
        } else {
            // Save as new plan
            saveNewLessonPlan(lessonName, formData, true);
        }
...
// 📝 Enhanced save function with auto-save flag
...
// 📥 Download lesson plan from modal
...
// 🗑️ Delete lesson plan from modal
function deleteLessonPlanFromModal(lessonName) {
    showConfirmationModal({
        ...
        onConfirm: () => {
            ...
            displayFolderModal(); // Refresh modal content
        }
    });
}
...
// 📋 Get current form data (enhanced)
...
// 🏷️ Generate lesson name (enhanced)
...
// 🎨 Add comprehensive folder system styles
...
// 🚀 Initialize the enhanced lesson plan folder system
function initializeLessonPlanFolderSystem() {
    try {
        ...
        // Enhance existing form submission if it exists
        enhanceExistingFormSubmission();
        
        console.log('📁 Enhanced Lesson Plan Folder System initialized successfully!');
        
        // Show initialization notification
        showLessonNotification("📁 Lesson Plan Folder System ready! Auto-save enabled.", "success");
...
// 🔗 Enhanced integration with existing form submission
function enhanceExistingFormSubmission() {
    try {
        // Look for existing submit button
        ...
        if (submitBtn && !submitBtn.dataset.enhanced) {
            ...
            submitBtn.onclick = function(e) {
                ...
                // Perform manual save first
                const saved = manualSaveLessonPlan();
                
                if (saved && originalHandler) {
                    // Call original handler if save was successful
                    originalHandler.call(this, e);
                } else if (saved && typeof updateContent === 'function') {
                    // If no original handler, just update content
                    updateContent();
                }
            };
        }
...
function addSaveButton() {
    try {
        ...
        if (!document.querySelector('#saveBtnStyles')) {
            const saveStyle = document.createElement('style');
            saveStyle.id = 'saveBtnStyles';

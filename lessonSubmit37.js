...
lessonplanSubmit.addEventListener('click', function (mySb) {
    ...
    downloadBtn.innerHTML = `<i class="bi bi-download"></i><span>Download PDF</span>`;
    
    const editLessonBtn = document.createElement("button");
    editLessonBtn.id = "editLessonBtn";
    editLessonBtn.className = "action-button secondary-btn";
    editLessonBtn.innerHTML = `<i class="bi bi-pencil-square"></i><span>Edit Lesson Plan</span>`;
    ...
    newLessonBtn.innerHTML = `<i class="bi bi-plus-circle"></i><span>Create New Lesson Plan</span>`;
    
    buttonContainer.appendChild(downloadBtn);
    buttonContainer.appendChild(editLessonBtn);
...
function userLoadd() {
    const wordToshow = document.getElementById("wordToshow");
    const videoDiv = document.getElementById("videoDiv");
    const adDiv = document.getElementById("adDiv");
    const refreshBtn = document.getElementById("refreshBtn");
    const downloadPdfBtn = document.getElementById('downloadPdfBtn');
    const gotoEdit = document.getElementById("gotoEdit");

    if (wordToshow) wordToshow.innerText = "Thanks! Your lesson plan will download shortly, please be patient";
    if (videoDiv) videoDiv.style.display = "block";
    if (adDiv) adDiv.style.display = "none";
    if (refreshBtn) refreshBtn.style.display = "block";
    if (downloadPdfBtn) {
        downloadPdfBtn.style.display = "block";
        downloadPdfBtn.innerText = "download lesson plan again";
    }
    if (gotoEdit) gotoEdit.style.display = "none";
}
...
function createNewLessonPlan() {
    try {
        ...
        // Remove action buttons from submit screen
        document.querySelectorAll('.back-to-folder-btn, .button-container').forEach(el => el.remove());
...
function initSimpleStorage() {
    setupAutoSaveSystem();
    showLessonNotification("Auto-save enabled", "success");
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initSimpleStorage);
} else {
    initSimpleStorage();
}
